...
//Code Here
let totalPopulation  = populations.reduce(function(a, b) {
  return a + b
}, 0)
...
let orderTotals = orders.map(order => {
  let taxedAmount = order["price"] * order["tax"]
  let total = taxedAmount + order["price"]
...
let bobsTotal = purchases.map(function (bobsOrder) {
  if (bobsOrder["owner"] === "Bob"){
    return bobsOrder["price"]
  }
})
